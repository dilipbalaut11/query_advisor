...
CREATE OR REPLACE FUNCTION pg_qualstats_get_index_ddl (
    v_quals_todo bigint[], amname TEXT)
    RETURNS text
AS $_$
DECLARE
    v_ddl text;
...
BEGIN
	...
	IF v_ddl != '' THEN
		v_ddl = pg_catalog.format('CREATE INDEX ON %s USING %I (%s)',
			pg_qualstats_get_qualnode_rel(v_qualnodeid), amname, v_ddl);
	END IF;

	return v_ddl;
END;
$_$ LANGUAGE plpgsql;       /* end of pg_qualstats_get_index_ddl */


CREATE OR REPLACE FUNCTION pg_qualstats_index_advisor_xx (
    min_filter integer DEFAULT 1000,
    min_selectivity integer DEFAULT 30,
    forbidden_am text[] DEFAULT '{}')
    RETURNS text[]
